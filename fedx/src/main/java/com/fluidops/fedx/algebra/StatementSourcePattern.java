...
/**
 * Represents statements that can produce results at a some particular
 * endpoints, the statement sources.
 * 
...
public class StatementSourcePattern extends FedXStatementPattern {
	...
	protected CloseableIteration<BindingSet, QueryEvaluationException> evaluate(BindingSet bindings,
		...
		try {
			...
			for (StatementSource source : sources) {
				...
				Endpoint ownedEndpoint = queryInfo.getFedXConnection().getEndpointManager()
				...
				if (t.usePreparedQuery()) {
					...
					union.addTask(new ParallelPreparedUnionTask(preparedQuery.getFirst(), t, ownedEndpoint, bindings,
	...
	protected CloseableIteration<BindingSet, QueryEvaluationException> handleStatementSourcePatternCheck(
			BindingSet bindings, List<StatementSource> sources) {
		// if at least one source has statements, we can return this binding set as
		// result
