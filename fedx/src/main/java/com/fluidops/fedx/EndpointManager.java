 ...
 * {@link Endpoint}s. Particular endpoints can be looked up by their id and
 * connection and all relevant information can be used.
 * 
...
public class EndpointManager {
	...
	protected static HashMap<String, Endpoint> endpoints = new HashMap<String, Endpoint>();
	...
	/**
	 * Repair the connection of the endpoint corresponding to the provided
	 * connection.
	 * 
	 * @param conn
	...
	public RepositoryConnection repairConnection(RepositoryConnection conn) throws RepositoryException {
		...
		try {
			...
			throw ExceptionUtil.changeExceptionMessage("Connection of endpoint " + e.getId() + " could not be repaired",
					ex, RepositoryException.class);
		}
	}
	...
	//		log.info("Repairing all connections after error.");
			...
			try {
				...
				throw ExceptionUtil.changeExceptionMessage("Connection of at least one endpoint could not be repaired.",
						r, RepositoryException.class);
			} finally {
				inRepair = false;
			}
		}
	 ...
	 * {@link FederationManager}. In addition, this method unregisters the
	 * {@link FederatedService} from Sesame
	 * 
	 * @param e the endpoint
	 * 
