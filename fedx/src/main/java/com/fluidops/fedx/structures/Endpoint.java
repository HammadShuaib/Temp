...
/**
 * Structure to maintain endpoint information, e.g. type, location, etc. For
 * initialization the underlying repository must be set using
 * {@link #setRepo(Repository, TripleSource, boolean)}.
 * <p>
 * 
...
public class Endpoint {
	 ...
	 * @author Andreas Schwarte
	 */
	public static enum EndpointClassification {
		Local, Remote;
	};
	...
	public static enum EndpointType {
		...
		RemoteRepository(Arrays.asList("RemoteRepository")), Other(Arrays.asList("Other"));
		...
		/**
		 * Returns true if the endpoint type supports the given format (e.g. mime-type).
		 * Consider as an example the SparqlEndpoint which supports format "api/sparql".
		 * 
		 * @param format
		 * @return
		 */
	 ...
	 * 
	 * @param id       the globally unique identifier, e.g. SPARQL_dbpedia351
	 * @param name     the name of this endpoint
	 * @param endpoint the endpoint, e.g. for SPARQL the URL
	 * @param type     the type, e.g. SPARQL, NativeStore
	 */
	public Endpoint(String id, String name, String endpoint, EndpointType type,
	 ...
	 * the underlying repository, i.e. until it is shutDown
	 * 
	 * @return
	 * 
	...
	/**
	 * Returns the size of the given repository, i.e. the number of triples. Can
	 * only be applied for types NativeStore and RemoteRepository.
	 * 
	 * @return
	 * @throws RepositoryException
	 */
	...
	public RepositoryConnection repairConnection() throws RepositoryException {
		...
		System.out.println("Repairing connection for endpoint " + id);

		if (conn != null) {
			try {
				conn.close();
			} catch (RepositoryException e) {
				System.err.printf("Connection of endpoint " + id + " could not be closed: " + e.getMessage());
			}
		}
		conn = repo.getConnection();
		System.out.println("Connection for endpoint " + id + " successfully repaired.");
		return conn;
	}
