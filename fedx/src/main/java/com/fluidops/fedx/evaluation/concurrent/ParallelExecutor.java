...
/**
 * Interface for any parallel cursor, i.e. result iterations. Implementations
 * can act as control for scheduler implementations, e.g.
 * {@link ControlledWorkerScheduler}. The common use case is to pass results
 * from the scheduler to the controlling result iteration.
 * 
...
public interface ParallelExecutor<T> extends Runnable {
	...
	/**
	 * Handle the result appropriately, e.g. add it to the result iteration. Take
	 * care for synchronization in a multithreaded environment
	 * 
	 * @param res
	 */
	...
	/**
	 * Inform the controlling instance that some job is done from a different
	 * thread. In most cases this is a no-op.
	 */
	public void done();
