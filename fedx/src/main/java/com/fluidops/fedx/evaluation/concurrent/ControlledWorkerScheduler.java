...
public class ControlledWorkerScheduler implements Scheduler {
	...
	public ControlledWorkerScheduler(int maxWorkerCount, String name) {
		...
		threadQueue_ = new LinkedBlockingQueue<WorkerThread>(maxWorkerCount_);
		schedulerName_ = name;
...
/*		long i=0;
		while(i<=999999999)
		{	i++;
	//	System.out.println("This is inorder to stop threads:"+i);
		}
		i=0;
		while(i<=999999999)
		{	
...
long m=0;
		while (!finished_.get()) {
			...
			//	log.debug("thread queue is running out of capacity, waiting for a free thread or allocatting new in "
	...
	public void schedule(Task task) {
		synchronized (taskQueue_) {
			...
			if (finished_.get()) {
				...
				//	log.debug("put task: " + task + ", priority: " + task.getPriority() + ", queue size: "
				//			+ taskQueue_.size());
				//}
						}
		}
	...
	public void shutdown() {
		...
		synchronized (threads_) {
			...
			// try { taskQueue_.put(new Runnable() { @Override public void run() {}}); }
			// catch (InterruptedException ignore) {}
