...
/**
 * Parallel executor for {@link FedXService} nodes, which wrap SERVICE
 * expressions.
 * 
...
//	public static Logger log = LoggerFactory.getLogger(ParallelServiceExecutor.class);
	 ...
	 * @param bindings
	 */
	public ParallelServiceExecutor(FedXService service, FederationEvalStrategy strategy, BindingSet bindings) {
		super();
	...
	public void run() {
		...
		service.getQueryInfo().getFederation().getScheduler().schedule(new ParallelServiceTask(taskPriotity),
				taskPriotity);
	}
