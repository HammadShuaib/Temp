...
//	public static Logger log = LoggerFactory.getLogger(QueryAlgebraUtil.class);
	...
	/**
	 * Return the {@link Value} of the variable which is either taken from the
	 * variable itself (bound) or from the bindingsset (unbound).
	 * 
	 * @param var
	 * @param bindings the bindings, must not be null, use {@link EmptyBindingSet}
	 *                 instead
	 * 
	...
	public static TupleExpr selectQuery(StatementPattern stmt, BindingSet bindings, FilterValueExpr filterExpr,
			Boolean evaluated) throws IllegalQueryException {
	 ...
	 * Pattern:
	 * 
	 * SELECT DISTINCT ?o_1 .. ?o_N WHERE { { s1 p1 ?o_1 FILTER ?o_1=o1 } UNION ...
	 * UNION { sN pN ?o_N FILTER ?o_N=oN }}
	 * 
	 * @param stmt
	 * @param unionBindings
	 * @return
	 ...
	 * wherever possible. Note that the free variables are added to the varNames set
	...
	/**
	 * Clone the specified variable and attach bindings, moreover change name of
