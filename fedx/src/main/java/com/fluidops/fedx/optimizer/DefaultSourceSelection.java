...
public class DefaultSourceSelection extends SourceSelection {
	 ...
	 */
	// protected Map<StatementPattern, List<StatementSource>> stmtToSources = new
	// ConcurrentHashMap<StatementPattern, List<StatementSource>>();
	 ...
	 * infrastructure of FedX. Note, that this method is blocking until every source
	 * is resolved.
	 * 
	...
	public void performSourceSelection(List<List<StatementPattern>> bgpGroups) {
		...
		if (remoteCheckTasks.size() > 0) {
			SourceSelectionExecutorWithLatch.run(queryInfo.getFederation().getScheduler(), this, remoteCheckTasks,
					cache);
		}
	...
	protected static class SourceSelectionExecutorWithLatch implements ParallelExecutor<BindingSet> {
		 ...
		 * tasks are completed. Synchronization is achieved by means of a latch. Results
		 * are added to the map of the source selection instance. Errors are reported as
		 * {@link OptimizationException} instances.
		 * 
		 * @param tasks
		 */
		public static void run(ControlledWorkerScheduler scheduler, DefaultSourceSelection sourceSelection,
				List<CheckTaskPair> tasks, Cache cache) {
		...
		private SourceSelectionExecutorWithLatch(ControlledWorkerScheduler scheduler,
		...
		/**
		 * Execute the given list of tasks in parallel, and block the thread until all
		 * tasks are completed. Synchronization is achieved by means of a latch
		 * 
		 * @param tasks
		 */
		private void executeRemoteSourceSelection(List<CheckTaskPair> tasks, Cache cache) {
			...
			List<Future<CloseableIteration<BindingSet, QueryEvaluationException>>> futures = new ArrayList<Future<CloseableIteration<BindingSet, QueryEvaluationException>>>(
					tasks.size());

			for (CheckTaskPair task : tasks) {
				futures.add(
						scheduler.schedule(new ParallelCheckTask(task.e, task.t, this), QueryInfo.getPriority() + 1));
			}
	...
	protected static class ParallelCheckTask
			implements Callable<CloseableIteration<BindingSet, QueryEvaluationException>> {
