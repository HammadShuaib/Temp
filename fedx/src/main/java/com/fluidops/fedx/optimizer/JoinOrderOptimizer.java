...
ForkJoinPool fjp =new ForkJoinPool(6);
		if (tupleExpr instanceof StatementSourcePattern)
			return fjp.submit(()->estimateCost((StatementSourcePattern) tupleExpr, joinVars)).join();
		if (tupleExpr instanceof ExclusiveStatement)
			return fjp.submit(()->estimateCost((ExclusiveStatement) tupleExpr, joinVars)).join();
		if (tupleExpr instanceof ExclusiveGroup)
			return  fjp.submit(()->estimateCost((ExclusiveGroup) tupleExpr, joinVars)).join();
		if (tupleExpr instanceof NJoin)
			return fjp.submit(()->estimateCost((NJoin) tupleExpr, joinVars)).join();
		if (tupleExpr instanceof NUnion)
			return fjp.submit(()->estimateCost((NUnion) tupleExpr, joinVars)).join();
		if (tupleExpr instanceof FedXService)
			return fjp.submit(()->estimateCost((FedXService) tupleExpr, joinVars)).join();
