...
//	public static Logger log = LoggerFactory.getLogger(FilterOptimizer.class);
	...
	public void meet(Filter filter) {
		...
		/*
		 * TODO idea: if we have a FILTER such as ?s='a' OR ?s='b' OR ?s='c' handle this
		 * appropriately
		 */
		...
		for (ValueExpr cond : conjunctiveExpressions) {
			...
			/*
			 * Determine if this filter is applicable for optimization. Currently only leaf
			 * expressions are applicable, i.e. not combined expressions.
			 */
	...
	protected class FilterExprInsertVisitor extends AbstractQueryModelVisitor<OptimizationException> {
		...
		private void handleFilter(FilterTuple filterTuple, FilterExpr expr) {
			 ...
			 * 
			 * if the filtertuple contains all vars of the filterexpr, we can evaluate the
			 * filter expr safely on the filterTuple
			 * 
			 * if there is no intersection of variables, the filter is irrelevant for this
			 * expr
			 * 
			 * if there is some intersection, we cannot remove the filter and have to keep
			 * it in the query plan for postfiltering
			 */
			int intersected = 0;
