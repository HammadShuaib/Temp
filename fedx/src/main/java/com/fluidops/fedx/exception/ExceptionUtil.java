...
public class ExceptionUtil {
	...
	/**
	 * Regex pattern to identify http error codes from the title of the returned
	 * document:
	 * 
	 * <code>
	...
	/**
	 * Trace the exception source within the exceptions to identify the originating
	 * endpoint. The message of the provided exception is adapted to "@
	 * endpoint.getId() - %orginalMessage".
	 * <p>
	 * 
	 * Note that in addition HTTP error codes are extracted from the title, if the
	 * exception resulted from an HTTP error, such as for instance "503 Service
	 * unavailable"
	 * 
	 * @param conn           the connection to identify the the endpoint
	 * @param ex             the exception
	 * @param additionalInfo additional information that might be helpful, e.g. the
	 *                       subquery
	 * 
	 * @return a modified exception with endpoint source
	 */
	public static QueryEvaluationException traceExceptionSource(EndpointManager endpointManager,
			RepositoryConnection conn, QueryEvaluationException ex, String additionalInfo) {
		...
		if (m.matches()) {
			////System.out.println("HTTP error detected for endpoint " + eID + ":\n" + message);
			message = "HTTP Error: " + m.group(1);
		} else {
			//System.out.println("No http error found, found: " + message);
		}
	 ...
	 * @return
	 */
	public static QueryEvaluationException traceExceptionSourceAndRepair(EndpointManager endpointManager,
			RepositoryConnection conn, QueryEvaluationException ex, String additionalInfo) {
	...
	/**
	 * Walk the stack trace and in case of SocketException repair the connection of
	 * the particular endpoint.
	 * 
	 * @param conn the connection to identify the endpoint
	 * @param ex   the exception
	 * 
	 * @throws FedXRuntimeException if the connection could not be repaired
	 */
	public static void repairConnection(EndpointManager endpointManager, RepositoryConnection conn, Exception ex)
		...
		while (cause != null) {
			if (cause instanceof SocketException) {
				try {
					...
					throw new FedXQueryException("Socket exception occured for endpoint "
							+ getExceptionString(e == null ? "unknown" : e.getId(), ex)
							+ ", all connections have been repaired. Query processing of the current query is aborted.",
							cause);
				} catch (RepositoryException e) {
					//System.out.println("Connection could not be repaired: ", e);
	...
	/**
	 * Return the exception in a convenient representation, i.e. '%msg% (%CLASS%):
	 * %ex.getMessage()%'
	 * 
	 * @param msg
	 * @param ex
	...
	/**
	 * If possible change the message text of the specified exception. This is only
	 * possible if the provided exception has a public constructor with String and
	 * Throwable as argument. The new message is set to 'msgPrefix.
