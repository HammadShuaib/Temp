...
/*
		...
		// location
		Model location = graph.filter(repNode,
				SimpleValueFactory.getInstance().createIRI("http://fluidops.org/config#SPARQLEndpoint"), null);
		...
		if (hasAdditionalSettings(graph, repNode)) {
			...
			if (graph.contains(repNode,
					SimpleValueFactory.getInstance().createIRI("http://fluidops.org/config#supportsASKQueries"),
	...
	protected boolean hasAdditionalSettings(Model graph, Resource repNode) {
		return graph.contains(repNode,
				SimpleValueFactory.getInstance().createIRI("http://fluidops.org/config#supportsASKQueries"), null);
	}
}
