...
/**
 * Provider for an Endpoint that uses a Sesame {@link SPARQLRepository} as
...
public class SPARQLProvider implements EndpointProvider {
	...
	public Endpoint loadEndpoint(RepositoryInformation repoInfo) throws FedXException {
		...
		try {
			 ...
			 * SPARQLFederatedService federatedService = new
			 * SPARQLFederatedService(repoInfo.getLocation(), null);
			 * federatedService.initialize(); FederatedServiceResolverImpl FederatedService
			 * Manager.getInstance().registerService(repoInfo.getName(), federatedService);
			 */
