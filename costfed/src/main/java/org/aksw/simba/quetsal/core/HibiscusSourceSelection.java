...
public void cache_ASKselection(StatementPattern stmt)
{
	 ...
	 */
	public ConcurrentHashMap<StatementPattern, List<StatementSource>> pruneSources(List<Set<Vertex>> dNFHyperVertices)
	{
		for (Set<Vertex> V : dNFHyperVertices)
		{
		 ...
		 if(V.size()>3)  //---- only consider those DNF groups having atleast 2 triple patterns
		 {
		   for (Vertex v:V) //vertex
		    {
				HashMap <StatementPattern, HashMap<StatementSource, ArrayList<String>>> stmtToLstAuthorities = new HashMap <StatementPattern, HashMap<StatementSource, ArrayList<String>>>(); 
			...
			if((v.inEdges.size()>1 && v.outEdges.size()>0) || (v.inEdges.size()>0 && v.outEdges.size()>1)) 
			 {
				...
				for(HyperEdge inEdge: v.inEdges) //has hyperedges or statement patterns
				 {
					 HashMap<StatementSource, ArrayList<String>> stmtSourceToAuthorities = new HashMap<StatementSource, ArrayList<String>> ();
 ...
 */
	private void doSourcePrunning(HashMap<StatementPattern, HashMap<StatementSource, ArrayList<String>>> stmtToLstAuthorities,ArrayList<String> authIntersectionSet) 
	{
		for(StatementPattern stmt:stmtToLstAuthorities.keySet())
		{
			HashMap<StatementSource, ArrayList<String>> stmtSourceToLstAuthorities = stmtToLstAuthorities.get(stmt);
