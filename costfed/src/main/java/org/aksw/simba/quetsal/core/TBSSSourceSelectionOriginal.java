...
public class TBSSSourceSelectionOriginal extends SourceSelection {
	...
	public HashMap<HyperEdge, StatementPattern> hyperEdgeToStmt = new HashMap<HyperEdge,StatementPattern>(); //Hyper edges to Triple pattern Map
	public List<HashMap<String, Vertex>> theDNFHyperVertices = new ArrayList<HashMap<String, Vertex>>(); // Maps of vertices in different DNF hypergraphs
	...
	private Vertex collectVertex(Var var, String label, HashMap<String, Vertex> v) {
	...
	public void performSourceSelection(List<List<StatementPattern>> bgpGroups) 
	{
		...
		{
			HashMap<String, Vertex> v = new HashMap<String, Vertex>();   //--Set (in fact) of all vertices used in our hypergraph. each subject, predicate and object of a triple pattern is one node until it is repeated
	...
	//	}
		...
		for (HashMap.Entry<StatementPattern, List<StatementSource>> stmtentry : stmtToSources.entrySet()) {
...
//	}
	 ...
	 */
	public ConcurrentHashMap<StatementPattern, List<StatementSource>> pruneSources(List<HashMap<String, Vertex>> dNFHyperVertices)
	{
		for (HashMap<String, Vertex> vs : dNFHyperVertices)
		{
			//System.out.println("--------------new DNF Graph---------");
			if (vs.size() > 3)  //---- only consider those DNF groups having at least 2 triple patterns
			{
				...
				{
					HashMap<StatementPattern, HashMap<StatementSource, List<String>>> stmtToLstAuthorities = new HashMap<StatementPattern, HashMap<StatementSource, List<String>>>(); 
					...
					if ((v.inEdges.size() > 1 && !v.outEdges.isEmpty()) || (!v.inEdges.isEmpty() && v.outEdges.size() > 1)) 
					{
						...
						{
							HashMap<StatementSource, List<String>> stmtSourceToAuthorities = new HashMap<StatementSource, List<String>> ();
	 ...
	 */
	private void doSourcePrunning(HashMap<StatementPattern, HashMap<StatementSource, List<String>>> stmtToLstAuthorities, List<String> authIntersectionSet) 
	{
		...
		HashMap<StatementPattern, HashMap<StatementSource, List<String>>> newStmtToLstAuthorities = new HashMap <StatementPattern, HashMap<StatementSource, List<String>>> ();
		...
		{
			HashMap<StatementSource, List<String>> newStmtSourceToLstAuthorities = new HashMap<StatementSource, List<String>>();
			// System.out.println("\n\n");
			// System.out.println("Intersection set: "+authIntersectionSet);
			HashMap<StatementSource, List<String>> stmtSourceToLstAuthorities = stmtToLstAuthorities.get(stmt);
		...
		HashMap<StatementPattern, HashMap<StatementSource, List<String>>> finalStmtToLstAuthorities = addAllCombinations(newStmtToLstAuthorities, allNewURIs);
	...
	private HashMap<StatementPattern, HashMap<StatementSource, List<String>>> addAllCombinations(
			HashMap<StatementPattern, HashMap<StatementSource, List<String>>> newStmtToLstAuthorities, List<String> allNewURIs) 
	{
		HashMap<StatementPattern, HashMap<StatementSource, List<String>>> hm = new HashMap<StatementPattern, HashMap<StatementSource, List<String>>>();
		TrieNode tn = Trie.constructTrie(allNewURIs, 100);
		
		for(StatementPattern stmt:newStmtToLstAuthorities.keySet())
		{
			HashMap<StatementSource, List<String>> newStmtSourceToLstAuthorities = new HashMap<StatementSource, List<String>>();
			HashMap<StatementSource, List<String>> stmtSourceToLstAuthorities = newStmtToLstAuthorities.get(stmt);
			for(StatementSource src:stmtSourceToLstAuthorities.keySet())
			{
	 ...
	 */
	private void doFinalPruning(HashMap<StatementPattern, HashMap<StatementSource, List<String>>> newStmtToLstAuthorities)
	{
	 ...
	 */
	private List<String> getIntersection(HashMap<StatementPattern, HashMap<StatementSource, List<String>>> newStmtToLstAuthorities)
	{
		...
		private void executeRemoteSourceSelection(List<CheckTaskPair> tasks, Cache cache) {
			...
			for (Future<Void> future : futures) {
				try {
					...
					System.err.println("Error during source selection. Thread got interrupted.");
					break;
				} catch (Exception e) {
					errors.add(e);
				}	
			...
			// check for errors:
			if (!errors.isEmpty()) {
				System.err.println(errors.size() + " errors were reported:");
				for (Exception e : errors)
					System.err.println(ExceptionUtil.getExceptionString("Error occured", e));
