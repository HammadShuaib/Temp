	 ...
	 for(int DNFkey:DNFgrps.keySet())
	  {
		...
		/*	StatementSourcePattern stmtNode = new StatementSourcePattern(stmt, queryInfo);
				...
				System.out.println("Total Triple Pattern-wise selected sources in step 1 of FedSum Source selection: " + triplePatternWiseSources+"--"+stmt);

			}
		
			else if (sources.size()==1) {
				stmt.replaceWith( new ExclusiveStatement(stmt, sources.get(0), queryInfo));
				System.out.println("Total Triple Pattern-wise selected sources in step 2 of FedSum Source selection: " + triplePatternWiseSources+"--"+stmt);
			...
			//		log.debug("Statement " + QueryStringUtil.toString(stmt) + " does not produce any results at the provided sources, replacing node with EmptyStatementPattern." );
				...
				System.out.println("Total Triple Pattern-wise selected sources in step 3 of FedSum Source selection: " + triplePatternWiseSources+"--"+stmt);
